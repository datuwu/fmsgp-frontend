...
import { temWareHouseRequestApi } from '@/core/api/temp-warehouse-request.api';
...
const Page: React.FunctionComponent<PageProps> = () => {
    ...
    const query = useQuery({
        queryKey: ['main-warehouse-request', id],
        queryFn: async () => {
            return await importMainWarehouseRequestApi.getById(Number(id));
        },
    });
    ...
    useDocumentTitle('Main Warehouse Request Detail');
        ...
        <div className="fade-in flex w-full flex-col gap-4">
            <div className="flex gap-2  rounded-lg bg-white p-4">
                <FieldBuilder
                    isPadding={false}
                    record={query.data}
                    title="Main Warehouse Request Detail"
                    extra={[
                        <>
                            {query.data?.approveStatus === TempWarehouseRequestApproveStatus.APPROVED && query.data?.warehouseForm === null && (
                                <CTAButton
                                    confirmMessage="Are you sure you want to create warehouse form?"
                                    ctaApi={async () => {
                                        return await warehouseFormApi.createMainWarehouseForm(Number(id));
                                    }}
                                    isConfirm
                                    extraOnSuccess={() => {
                            ...
                            {query.data?.approveStatus === TempWarehouseRequestApproveStatus.PENDING && isWarehouseStaff && (
                                <>
                                    <ModalBuilder
                                        btnLabel="Approve"
                                        btnProps={{
                                    ...
                                    >
                                        {(closeChildren) => {
                                            return (
                                                <FormBuilder<ICreateImportMainWarehouseRequestDto>
                                                    apiAction={async (data) => {
                                                        return await importMainWarehouseRequestApi.approval(data);
                                                    }}
                                                    isDebug
                                                    title="Approve Main Warehouse Request"
                                                    schema={{
                                                        approveStatus: Joi.number().required(),
                                                        approveExecutionDate: Joi.any(),
                                                        id: Joi.number().required(),
                                                        rejectReason: Joi.string().allow(''),
                                                        statusUpdateDate: Joi.string().allow(''),
                                                    }}
                                                    defaultValues={{
                                                        ...
                                                        rejectReason: '',
                                                        statusUpdateDate: moment().format('YYYY-MM-DDTHH:mm:ss'),
                                                    }}
                                                    onExtraSuccessAction={() => {
                                                        toast.success('Approve main warehouse request successfully');
                                                        query.refetch();
                                                        closeChildren();
                                                    }}
                                                    fields={[
                                    ...
                                    >
                                        {(closeChildren) => {
                                            return (
                                                <FormBuilder<ICreateImportMainWarehouseRequestDto>
                                                    apiAction={async (data) => {
                                                        return importMainWarehouseRequestApi.approval(data);
                                                    }}
                                                    title="Reject Main Warehouse Request"
                                                    schema={{
                                                        approveStatus: Joi.number().required(),
                                                        approveExecutionDate: Joi.any(),
                                                        id: Joi.number().required(),
                                                    ...
                                                    onExtraSuccessAction={() => {
                                                        toast.success('Reject main warehouse request successfully');
                                                        query.refetch();
                                                        closeChildren();
                                                    }}
                                                    fields={[
                    ...
                    fields={[
                        ...
                        {
                            key: 'approveExecutionDate',
                            title: 'Approve Execution Date',
                            type: FieldType.TIME_DATE,
                            span: 3,
                        },
                    ]}
                            ...
                            <div>
                                {query.data?.warehouseForm?.warehouseFormMaterials.some(
                                    ...
                                    <>
                                        <CTAButton
                                            ctaApi={async () => {
                                                return await warehouseFormApi.mainImportFormStatus(query.data?.warehouseForm.id);
                                            }}
                                            ...
                                            {(close) => (
                                                <div>
                                                    <FormBuilder<IMainImportFormStatusFormDto>
                                                        apiAction={(data) => {
                                                            return warehouseFormApi.getApiMainImportFormStatus({
                                                                id: data.id,
...
export const Route = createFileRoute('/_admin-layout/dashboard/import-main-warehouse-request/$id/')({
    component: Page,
});
